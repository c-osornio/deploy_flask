...
# Validate
    ...
    def validate_user_reg_data(data):
        ...
        if len(data['first_name']) < 3:
            flash("Your first name must be at least three characters long.", "registration")
            is_valid = False
        if not re.match("^[A-Za-z]*$", data['first_name']):
            flash("Your first name can only contain letters." , "registration")
            is_valid = False
        if len(data['last_name']) < 3:
            flash("Your last name must be at least three characters long." , "registration")
            is_valid = False
...
# Validate edit form data
    ...
    def validate_user_edit_data(data):
        ...
        is_valid = True
        if len(data['first_name']) < 3:
            flash("Your first name must be at least three characters long.", "edit")
            is_valid = False
        if not re.match("^[A-Za-z]*$", data['first_name']):
            flash("Your first name can only contain letters." , "edit")
            is_valid = False
        if len(data['last_name']) < 3:
            flash("Your last name must be at least three characters long." , "edit")
            is_valid = False
        if not re.match("^[A-Za-z]*$", data['last_name']):
            flash("Your last name can only contain letters." , "edit")
            is_valid = False
        if not EMAIL_REGEX.match(data['email']):
        ...
        if user.email != data['email']:
            if User.get_user_by_email(data['email']):
                flash('Email address is already in use.', "edit")
                is_valid = False
        return is_valid